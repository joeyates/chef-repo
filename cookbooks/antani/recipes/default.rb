...
gitosis_users = all_users.map { | u | u[ 'logon' ] }
group 'gitosis' do
  members gitosis_users
  append true
end
...
if not File.directory?( "#{ node[ :gitosis ][ :home ] }/.ssh/id_dsa" )

  execute "ssh-keygen -t dsa -f #{ node[ :gitosis ][ :home ] }/.ssh/id_dsa -N '' -C gitosis" do
    creates "#{ node[ :gitosis ][ :home ] }/.ssh/id_dsa"
    cwd node[ :gitosis ][ :home ]
    user node[ :gitosis ][ :user ]
    group node[ :gitosis ][ :user ]
  end

  execute "gitosis-init < #{ node[ :gitosis ][ :home ] }/.ssh/id_dsa.pub" do
